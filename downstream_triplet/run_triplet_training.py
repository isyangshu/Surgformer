...
from downstream_triplet.datasets_triplet import build_dataset
from downstream_triplet.engine_for_triplet import (
    train_one_epoch,
...
def get_args():
    parser = argparse.ArgumentParser(
        "SurgVideoMAE fine-tuning and evaluation script for video triplet recognition",
        add_help=False,
    )
    ...
    parser.add_argument("--long_side_size", type=int, default=224)
    ...
    parser.add_argument(
        "--data_path",
        default="/home/syangcw/datasets/CholecT50",
        type=str,
        help="dataset path",
    )
    parser.add_argument(
...
def main(args, ds_init):
    ...
    if args.eval:
        ...
        test_stats = final_triplet_test(data_loader_test, model, device, preds_file)
        print("Save Files: ", preds_file)
        torch.distributed.barrier()
        if global_rank == 0:
            print("Start merging results...")
            mAP_i, mAP_v, mAP_t, mAP_iv, mAP_it, mAP_ivt = merge(args.output_dir, num_tasks)
            print(
                f"Accuracy of the network on the {len(dataset_test)} test videos: mAP-i: {mAP_i:.2f}%, mAP-v: {mAP_v:.2f}%, mAP-t: {mAP_t:.2f}%, mAP-iv: {mAP_iv:.2f}%, mAP-it: {mAP_it:.2f}%, mAP-ivt: {mAP_ivt:.2f}%"
            )
            log_stats = {"Final mAP-i": mAP_i, "Final mAP-v": mAP_v, "Final mAP-t": mAP_t, "Final mAP-iv": mAP_iv, "Final mAP-it": mAP_it,
                        "Final mAP-ivt": mAP_ivt}
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            print(
                f"mAP of the network on the {len(dataset_val)} val videos: {test_stats['mAP']:.2f}%"
            )
            if max_mAP < test_stats["mAP"]:
                max_mAP = test_stats["mAP"]
                max_epoch = epoch
                if args.output_dir and args.save_ckpt:
                    utils.save_model(
                        args=args,
            ...
            print(
                f"Max accuracy: {max_mAP:.2f}%" + "   Max Epoch: " + str(max_epoch)
            )
            if log_writer is not None:
                log_writer.update(val_mAP=test_stats["mAP"], head="perf", step=epoch)
