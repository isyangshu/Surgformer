...
def main():
    ...
    for video_id in VIDEO_NAMES:
        ...
        if vid_id in TRAIN_NUMBERS:
            train_save_dir = os.path.join(ROOT_DIR, 'labels_pkl', 'train', 'single')
            os.makedirs(train_save_dir, exist_ok=True)
            with open(os.path.join(train_save_dir, video_id + '.pickle'), 'wb') as file:
                pickle.dump(pkl, file)
        elif vid_id in VAL_NUMBERS:
            val_save_dir = os.path.join(ROOT_DIR, 'labels_pkl', 'val', 'single')
            os.makedirs(val_save_dir, exist_ok=True)
            with open(os.path.join(val_save_dir, video_id + '.pickle'), 'wb') as file:
                pickle.dump(pkl, file)
        elif vid_id in TEST_NUMBERS:
            test_save_dir = os.path.join(ROOT_DIR, 'labels_pkl', 'test', 'single')
            os.makedirs(test_save_dir, exist_ok=True)
            with open(os.path.join(test_save_dir, video_id + '.pickle'), 'wb') as file:
                pickle.dump(pkl, file)
...
if __name__ == "__main__":
    ...
    file = open('/Users/yangshu/Downloads/CholecT50/labels_pkl/train/single/VID01.pickle', 'rb')
