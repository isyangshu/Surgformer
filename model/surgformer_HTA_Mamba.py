# V0：LN+SA+LN+TA & LN+VSS (兼容cls token) -> LN+MLP
import torch
...
class SSVideo(nn.Module):
    ...
    def forward_core(self, x: torch.Tensor, cls_token: torch.Tensor):
        B, C, H, W = x.shape
        L = H * W
        K = 4
    ...
    def forward(self, x: torch.Tensor, T, K, **kwargs):
        cls_token = x[:, 0, :].unsqueeze(1)
        h = w = int(math.sqrt(K))
        t = int(math.sqrt(T))
        x_st = x[:, 1:, :]
        x = rearrange(x_st, "b (h w t) c -> b c t h w", t=T, h=h)
        x = rearrange(x, "b c (t1 t2) h w -> b (t1 h) (t2 w) c", t1=t, t2=t)
        B, H, W, C = x.shape
        ...
        # add cls token for scan
        y1, y2, y3, y4 = self.forward_core(x, cls_token)
        ...
        out = rearrange(out, "b (t1 h) (t2 w) c -> b c (t1 t2) h w", t1=t, t2=t)
        out = rearrange(out, "b c t h w -> b (h w t) c")
        return torch.cat((cls_token, out), dim=1)


class VSSVideo(nn.Module):
    def __init__(
        self,
        hidden_dim: int = 0,
        drop_path: float = 0,
        ...
        **kwargs,
        ...
        self.mamba_st = SSVideo(d_model=hidden_dim, dropout=attn_drop_rate, d_state=d_state, **kwargs)
        self.drop_path = DropPath(drop_path)

    def forward(self, input: torch.Tensor, T, K):
        x = input + self.drop_path(self.mamba_st(self.ln_1(input), T, K))
        return x
...
class Block(nn.Module):
    ...
    def forward(self, x, B, T, K):
        ...
        x = x + self.drop_path(self.mlp(self.norm2(x + x_st)))  # 通过MLP学习时序对应的cls_token?
...
@register_model
def surgformer_HTA_Mamba(pretrained=False, pretrain_path=None, **kwargs):
...
if __name__ == "__main__":
    ...
    model = surgformer_HTA_Mamba(
        pretrained=True,
        pretrain_path="/home/syangcw/Surgformer/pretrain_params/timesformer_base_patch16_224_K400.pyth"
    ...
    # x1 = torch.ones((1, 4, 4))
    # x2 = torch.ones((1, 4, 4)) * 2
    # x3 = torch.ones((1, 4, 4)) * 3
    # x4 = torch.ones((1, 4, 4)) * 4
    # x = torch.cat((x1, x2, x3, x4), dim=0)
    # print(x)
    # x = rearrange(x, "(t1 t2) h w -> (t1 h) (t2 w)", t1=2)
    # print(x)

    # x1 = torch.ones((1, 16))
    # x2 = torch.ones((1, 16)) * 2
    # x3 = torch.ones((1, 16)) * 3
    # x4 = torch.ones((1, 16)) * 4
    # x = torch.cat((x1, x2, x3, x4), dim=0)
    # print(x.shape)
    # x = rearrange(x, "t k -> (t k)")
    # print(x)
    # x = rearrange(x, " (m n) -> m n", m=8)
    # print(x)